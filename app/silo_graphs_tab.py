...
class SiloGraphsTab(QWidget):
    ...
    def show_silo_select_dialog(self):
        try:
            dialog = SiloHotspotsDialog(
                self.ctx.db_conn,
                self,
                self.ctx.temp_threshold_spinbox.value(),
                last_silo=self.selected_silo,
                last_points=self.selected_points
            )
            dialog.start_date.setDate(self.ctx.start_date_edit.date())
            dialog.end_date.setDate(self.ctx.end_date_edit.date())
    ...
    def build_multi_silo_graph(self):
        try:
            if not self.selected_silo or not self.selected_points:
                self.ctx.status_label.setText("Выберите силос и горячие точки")
                return

            plot_series = {}
            for susp, sensor in self.selected_points:
                history_data = get_sensor_history_with_dates(
                    self.ctx.db_conn, self.selected_silo, susp, sensor,
            ...
            self.ctx.status_label.setText(f"Построен график для {len(self.selected_points)} точек")

            set_user_setting(self.ctx.db_conn, 'graph_start_date', self.graph_start_date)
            set_user_setting(self.ctx.db_conn, 'graph_end_date', self.graph_end_date)
        except Exception as e:
    ...
    def on_silo_graph_point_selected(self, date, temp, series):
        try:
            ...
            if self.ctx.date_format_with_year:
