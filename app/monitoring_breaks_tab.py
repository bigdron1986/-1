...
class MonitoringTabMixin:
    """Методы вкладки 3: Мониторинг изменений"""
    ...
    def update_monitoring_view(self):
        ...
        try:
            ...
            for row_idx, change in enumerate(changes):
                ...
                status_item = QTableWidgetItem(status)
                if delta > 0:
                    status_item.setBackground(QColor(self.color_hotspot))
                else:
                    status_item.setBackground(QColor(self.color_normal))
                self.changes_table.setItem(row_idx, 7, status_item)
    ...
    def on_monitoring_cell_clicked(self, row, column):
        """Клик по таблице мониторинга - показать график за неделю от последней даты периода"""
        try:
            ...
            # Вычислить даты: неделя от последней даты выбранного периода
            from datetime import datetime, timedelta
            ...
            # Получить историю датчика за неделю
            history_data = get_sensor_history_with_dates(
                self.db_conn, silo, suspension, sensor, start_date_str, end_date_str
            )
...
class BreaksTabMixin:
    """Методы вкладки 4: Обрывы датчиков"""
