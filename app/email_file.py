...
class EmailFileMixin:
    """Методы: Email, Загрузка файлов, Общие утилиты"""
    ...
    def load_reports_from_paths(self, file_paths):
        """Загрузить и обработать несколько файлов отчетов"""
        success_count = 0
        skip_count = 0
        error_count = 0
        ...
        for file_path in file_paths:
            try:
                ...
                insert_readings(self.db_conn, readings)
                success_count += 1
    ...
    def show_email_menu(self):
        ...
        settings_action.triggered.connect(self.email_settings_dialog)
        download_action.triggered.connect(self.email_download_dialog)
        menu.exec(self.email_button.mapToGlobal(self.email_button.rect().bottomLeft()))
