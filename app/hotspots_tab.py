...
class HotspotsTab(QWidget):
    """Вкладка 1: Горячие точки"""
    ...
    def _build(self):
        ...
        self.show_year_check.setChecked(self.ctx.date_format_with_year)
        self.show_year_check.stateChanged.connect(self.update_data_view)
        self.ctx.show_year_check = self.show_year_check
        ...
        self.hotspot_color_btn.setStyleSheet(f"background-color: {self.ctx.color_hotspot};")
        ...
        self.error_color_btn.setStyleSheet(f"background-color: {self.ctx.color_error};")
        ...
        self.ctx.main_splitter = self.main_splitter

        layout.addWidget(self.main_splitter)

        if self.ctx.config.get("splitter_sizes"):
            self.main_splitter.setSizes(self.ctx.config["splitter_sizes"])
    ...
    def update_data_view(self):
        try:
            ...
            if hasattr(self.ctx, 'main_tabs') and self.ctx.main_tabs.currentIndex() == 5:
            ...
            suspension = self.ctx.suspension_combo.currentText()
            plot_type = self.ctx.plot_type_combo.currentText()
            threshold = self.ctx.temp_threshold_spinbox.value()
            ...
            if silo != "Все силосы":
                if plot_type == "Средняя температура":
                    ...
                    if suspension != "Все подвески":
                        ...
                        avg_data = get_average_temp_by_suspension(self.ctx.db_conn, silo, susp_num, start_date, end_date)
                        plot_series[f"Средняя t° (подв. {susp_num})"] = avg_data
                        self.ctx.status_label.setText(f"График средней t° для силоса {silo}, подвески {susp_num}")
                    else:
                        avg_data = get_average_temp_by_silo(self.ctx.db_conn, silo, start_date, end_date)
                        plot_series[f"Средняя t° (силос {silo})"] = avg_data
                        self.ctx.status_label.setText(f"График средней t° для силоса {silo}")
                    self.plot_widget.plot_data(plot_series)

                elif suspension != "Все подвески":
                    self.hot_spots_table.setRowCount(0)
                    susp_num = int(suspension)
                    all_sensors_data = get_readings(self.ctx.db_conn, silo=silo, start_date=start_date, end_date=end_date)
                    plot_series = {}
                    ...
                    self.plot_widget.plot_data(plot_series)
                    self.ctx.status_label.setText(f"График для силоса {silo}, подвески {susp_num}")
            ...
            self.ctx.breaks_tab.populate_breaks_table(silo if silo != "Все силосы" else None, start_date, end_date)
    ...
    def populate_hotspots_table(self, silo, date, threshold):
        ...
        hot_spots = get_hot_spots_for_date(self.ctx.db_conn, silo if silo != "Все силосы" else None, date, threshold)
        self.hot_spots_table.setRowCount(0)

        if not hot_spots:
            self.hot_spots_label.setText(f"🔥 Горячих точек: 0 (порог: {threshold}°C)")
            self.ctx.status_label.setText(f"Горячих точек не найдено за {date}")
            return
        ...
        for row_idx, row_data in enumerate(hot_spots):
            ...
            self.highlight_row(row_idx, QColor(self.ctx.color_hotspot))
            ...
            if prev_date:
                prev_temp = get_sensor_temperature_on_date(self.ctx.db_conn, silo_name, suspension, sensor, prev_date)
        ...
        self.ctx.status_label.setText(f"Найдено {len(hot_spots)} горячих точек за {date}")
    ...
    def update_hotspot_leader_label(self, date, threshold):
        try:
            leader_info = get_leader_change_info(self.ctx.db_conn, date, threshold)
            if not leader_info:
    ...
    def on_hotspot_clicked(self, row, column):
        ...
        history_data = get_sensor_history_with_dates(self.ctx.db_conn, silo, suspension, sensor, start_date, end_date)
    ...
    def on_threshold_changed(self, value):
        self.ctx.temp_threshold = value
        set_user_setting(self.ctx.db_conn, 'temp_threshold', str(value))
        self.update_data_view()

    def choose_color(self, color_type):
        current_colors = {
            'hotspot': self.ctx.color_hotspot, 'error': self.ctx.color_error,
            'normal': self.ctx.color_normal, 'warning': self.ctx.color_warning,
        }
        ...
        if color.isValid():
            ...
            if color_type == 'hotspot':
                self.ctx.color_hotspot = hex_color
                self.hotspot_color_btn.setStyleSheet(f"background-color: {hex_color};")
            elif color_type == 'error':
                self.ctx.color_error = hex_color
                self.error_color_btn.setStyleSheet(f"background-color: {hex_color};")
            elif color_type == 'normal':
                self.ctx.color_normal = hex_color
            elif color_type == 'warning':
                self.ctx.color_warning = hex_color
            set_user_setting(self.ctx.db_conn, f'color_{color_type}', hex_color)
            self.update_data_view()
    ...
    def export_table_dialog(self, table, table_type):
        if table.rowCount() == 0:
            self.ctx.status_label.setText("Таблица пуста, нечего экспортировать.")
            return
        ...
        if file_path:
            try:
                ...
                self.ctx.status_label.setText(f"Таблица экспортирована в {file_path}")
            except Exception as e:
                self.ctx.status_label.setText(f"Ошибка экспорта таблицы: {e}")
    ...
    def save_graph_dialog(self):
        ...
        if file_path:
            try:
                ...
                self.ctx.status_label.setText(f"График сохранен в {file_path}")
            except Exception as e:
                self.ctx.status_label.setText(f"Ошибка сохранения: {e}")
    ...
    def reset_database(self):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            cursor = self.ctx.db_conn.cursor()
            cursor.execute("DELETE FROM readings")
            self.ctx.db_conn.commit()
            self.ctx.email_file_mixin.populate_silo_filter()
            self.ctx.email_file_mixin.update_date_range()
            self.update_data_view()
            self.ctx.status_label.setText("✅ База данных очищена")
