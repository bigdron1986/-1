...
class HottestSensorsTab(QWidget):
    ...
    def on_silo_clicked(self, silo_name, date):
        dialog = SiloCommentDialog(self, silo_name, date, self.ctx.db_conn)
        ...
        if reply == QMessageBox.StandardButton.Yes:
            ...
            if reply2 == QMessageBox.StandardButton.Yes:
                try:
                    ...
                    self.ctx.db_conn = setup_database(db_path)
                    logging.info("База данных создана заново")
                    self.ctx.status_label.setText("🗑️ База данных сброшена")
    ...
    def update_hottest_sensors_view(self, selected_date=None, refresh_timeline=True):
        try:
            ...
            if not selected_date:
                all_dates = get_available_dates(self.ctx.db_conn)
                if not all_dates:
            ...
            if refresh_timeline and hasattr(self, 'timeline_slider'):
                ...
                try:
                    ...
                    dates_range = get_date_range_for_slider(self.ctx.db_conn, start_date_str, selected_date)
            ...
            silos_delta_data = get_all_silos_delta_for_date(self.ctx.db_conn, selected_date)
            ...
            for silo in silos_delta_data.keys():
                comments[silo] = has_any_comment(self.ctx.db_conn, silo)
    ...
    def check_leader_changes(self):
        try:
            last_processed = get_last_processed_leader_date(self.ctx.db_conn)
            min_date, max_date = get_date_range(self.ctx.db_conn)
            ...
            if start_date <= max_date:
                ...
                changes_count = check_leader_changes_for_period(
                    self.ctx.db_conn, start_date, max_date, self.ctx.temp_threshold
                )
                if changes_count > 0:
                    logging.info(f"Найдено {changes_count} смен лидера")
                    self.ctx.status_label.setText(f"🔄 Найдено {changes_count} смен лидера")
        except Exception as e:
